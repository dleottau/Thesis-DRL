...
Pr(1) = Pr(1) + cosd(Pr(3))*Vr(1)*dt + cosd(Pr(3)+90)*Vr(2)*dt; % No cambiar Vr(2)
Pr(2) = Pr(2) + sind(Pr(3))*Vr(1)*dt + sind(Pr(3)+90)*Vr(2)*dt; % No cambiar Vr(2)
% -------------------------------------------------------------------------
Pr(3) = Pr(3) + Vr(3)*dt;
% -------------------------------------------------------------------------
...
elseif sum(Pb-Pt) < 0.1
    bet = 0; %particular case where ball y too close to desired target
end
...
if collision, %for now, just a cilindrical robot shape with radious 60mm
    % ---------------------------------------------------------------------
    Vb = ( abs(Vr(3))+1.5*abs(Vr(1))+78.24*noiseBall(2) ) * cos( dirb*pi()/180-(atan2(0,Vr(1))+Pr(3)*pi()/180) );
    % ---------------------------------------------------------------------
