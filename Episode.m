function [ Vxr,ro,fi,gama,Pt,Pb,Pr,Vb,total_reward,steps,Q,trace,fitness,btd,Vavg,Vth,time,faults] = Episode( w, maxDistance, Q , alpha, gamma,epsilon,p, statelist,actionlist,Ts,Vth,th_max, lambda, trace,NOISE)
...
Vyrmax=100;
Vthetamax=100;
Vr_max=[100 30 40]; %x,y,rot Max Speed achieved by the robot
...
[Pr(i,:), Ptr(i,:), Pbr(i,:), alfa, fi(i,1), gama(i,1), ro(i,1), Vb(i,1), dirb(i,1), Pb(i,:)]=mov(Ts,Pr,Pt,Pb,Vxr,Vyr,Vtheta,Vxrmax,Vyrmax,Vthetamax,Vb,dirb,Fr, clipDLF(randn(2,1)*NoiseBall,-2,2));
dV=0;

x = [Pr(i,1),Pb(i,1),Vb(i,1),Vxr(i,1),ro(i,1),dV,gama(i,1),fi(i,1)];

% Get velocity FLC 
[Vx_FLC, Vyr, Vtheta ] = FLC_dribbling (w,x,Voffset,Vr_max);
...
%for i=1:maxsteps    
 while 1  
    ...
    Vr = clipDLF(Vr,Voffset,Vr_max(1)); 
    Vxr(i,1)=Vr;
    
    % Updating kinematincs model 
    [Pr(i,:), Ptr(i,:), Pbr(i,:), alfa, fi(i,1), gama(i,1), ro(i,1), Vb(i,1), dirb(i,1), Pb(i,:)]=mov(Ts,Pr(i-1,:),Pt,Pb(i-1,:),Vxr(i,1),Vyr,Vtheta,Vxrmax,Vyrmax,Vthetamax,Vb(i-1,1),dirb(i-1,1),Fr, clipDLF(randn(2,1)*NoiseBall, -2, 2) );
    ...
    % Get velocity FLC 
    [Vx_FLC, Vyr, Vtheta ] = FLC_dribbling (w,x_obs,Voffset,Vr_max);
    ...
    if sum(thres)~=0
        faults = faults+1;
        fitness = fitness - ( (1-Vr/Vr_max(1)) + sum(thres .* [pho_  abs(gamma_) abs(phi_)] .*1/th_max) );
        %fitness(3) = fitness(3) - ( (2-Vr/Vr_max(1)) * sum(thres.*[pho_  yfi(i,1)].*[1/th_max 1/th_max/4]) );
    else
        fitness = fitness - (1-Vr/Vr_max(1));
