...
[cumGoals, elapsedTime] = RUN_SCRIPT(x0,RUNS,stringName);
f=-cumGoals;

if ~opti
    disp('-');
    disp(['cumGoals:',num2str(cumGoals), '; ElapsedTime:',num2str(elapsedTime)]);
    disp('-');
    toc
    %matlabpool close;
else
    disp(['cumGoals:',num2str(cumGoals), '; ElapsedTime:',num2str(elapsedTime), '; ' stringName]);
